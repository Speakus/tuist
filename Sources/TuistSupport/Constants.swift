...
public enum Constants {
    ...
    public static let version: String = getAppVersion()
...
private extension Constants {
    // based on https://stackoverflow.com/a/33177600/751932
    ...
    // TODO: embed Info.plist into executable
    // https://stackoverflow.com/a/28681655/751932
    private static let isInfoPlistEmbeded = false

    static func getAppVersion() -> String {
        let safeResult = "3.28.1"
        if isInfoPlistEmbeded {
            guard let infoDictionary = Bundle.main.infoDictionary else {
                precondition(amIBeingDebugged(), "use script makeTuist.sh to build")
                return safeResult
            }
            guard let ver = infoDictionary[kCFBundleVersionKey as String] else {
                precondition(amIBeingDebugged(), "use script makeTuist.sh to build")
                return safeResult
            }
        ...
        let verInfoPath = execPath.deletingLastPathComponent().appendingPathComponent("tuistversion.txt", isDirectory: false)
        do {
            return try String(contentsOf: verInfoPath)
        } catch {
            fatalError(verInfoPath.absoluteString + ":" + error.localizedDescription)
        }
    }
}
